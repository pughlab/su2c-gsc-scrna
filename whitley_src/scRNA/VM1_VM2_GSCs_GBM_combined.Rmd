...
do_fisher_multi <- function(query_set, pathways_list, master_set, FDR = 0.10, n_cores = 1) {
  ...
  do_fisher_single <- function(x) {
    return(fisher.test.sets(set1 = query_set, set2 = pathways_list[[x]], master.set = master_set)$p.value)
  }
  if (n_cores == 1) {
...
data.frame(VM_loadings)

get_top_loadings <- function(gene_loadings, i, n = 50L) {
  loadings_i <- gene_loadings[,i]
  top_n_up <- names(sort(loadings_i)[1:n])
  top_n_down <- names(sort(loadings_i, decreasing = TRUE)[1:n])
