...
output:
    ...
    toc_depth: 4
params:
  inp_data: '~/projects/su2c_v2/data/preprocessed/scRNA/Wang_2019_CancerDiscov/Wang_scRNA_full_seurat.rds'
...
if (feats_plot == 'NULL') {
  feats_plot <- c(paste0('chr.', c(7,10)), 'cahoy_astrocyte_AUC', 'cahoy_oligodendrocyte_AUC', 'cahoy_neuron_AUC', 'Zhong_Microglia_upreg_AUC',
                  'nowakowski_Endothelial_upreg_AUC', 'Neftel_Macrophage_AUC', 'RNA_GSC_line_classifier_AUC', 'cluster', 'SampleID')
}
...
5 is likely endothelial, 4 is likely oligodendrocyte, 3 is likely microglia. It is likely that there are astrocytes in the data, given Figure 1A in Wang et al. 2019, but for now we can't distinguish them (clusterwise) from teh rest of the tumour. If we really wanted to be sure we were only looking at tumour cells, we'd look for things with an amplification of chromosome 7 and deletion of 10.

``` {r}
inp_data@meta.data$is.glioma <- !(inp_data@meta.data$cluster %in% c('3', '4', '5'))
...
seurat_obj <- scoring_subroutine(seurat_obj, genesets, output_dir, 'Wang_2019_scRNA_filtered')

# Final dims
dim(seurat_obj@data)

saveRDS(seurat_obj, file = file.path(output_dir, 'Wang_2019_scRNA_filtered.rds'))
```
